...
<div class="container">
                ...
                {!! Form::open(['url' => route('order-post'), 'data-parsley-validate', 'id' => 'payment-form']) !!}
                    ...
                    <div class="form-group" id="product-group">
                        {!! Form::label('plane', 'Select Plan:') !!}
                          {{-- {!! Form::select('plane', ['100' => 'Game ($50)','500' => 'Movie ($100)'], null, [
                            'class'                       => 'form-control',
                            'required'                    => 'required',
                            'data-parsley-class-handler'  => '#product-group'
                            ]) !!}  --}}
                         <select name="plane" class="form-control" required data-parsley-class-handler="#product-group" id="check">
    ...
    @if (Auth::user()->subscribed('price_1KFdmIJ8mQDH5roiIHMjr91z')) 
    {{-- {{ $info = Auth::user()->asStripeCustomer() }} --}}
        @php
            $transactions = Auth::user()->balanceTransactions() 
        @endphp
    ...
    {{-- @php --}}
        ...
        {{-- // $subscriptions = User::where('id', session('id'))->subscriptions()->active()->first(); 

        //  User::where('domain', tenant('id'))->subscriptions()->active()->first()->;
